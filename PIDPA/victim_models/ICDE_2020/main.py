...
class ICDE_2020(object):
    ...
    def poison(self):
        ...
        # y2 = [math.log(a, 10) for a in self.agent.envx.cost_trace_overall]
        # plt.plot(x, y2, marker='x')
        # plt.savefig(self.experiment_folder_path + '/' + str(self.agent.number)+"_"+ str(self.config["model"]["algorithm"][0]["parameters"]["number"]) +"poison_freq.png", dpi=120)
        # plt.clf()
        # plt.close()
    ...
    def evaluation(self):
        ...
        # plt.plot(x, y2, marker='x')
        # plt.savefig(self.experiment_folder_path + '/' + str(self.agent.number) + "_" + str(
        #     self.config["model"]["algorithm"][0]["parameters"]["number"]) + "after_poison_freq.png", dpi=120)
        # plt.clf()
        # plt.close()
